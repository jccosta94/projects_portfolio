...
**The architectural insight — verticalisation is a configuration concern, not a code concern.** Every engagement is the same shape: Cowork desktop install + configured `CLAUDE.md` + seeded memory + an MCP fleet matched to the operator's existing tools + the 11-skill core library + vertical-specific extensions + a small number of scheduled tasks for recurring digests. **None of this is custom code; all of it is configuration.** Adding a vertical = adding ~6-7 skills, ~2-3 MCP-server bindings, and a vertical-tuned HITL gate model. Same Cowork, same Claude, different wiring on either side.
