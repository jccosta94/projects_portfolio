...
**Live consulting practice.** Architected three vertical templates on top of [Claude Cowork](https://claude.com/cowork) — same desktop install, same Claude, three different operator cockpits — by varying only the MCP fleet, the skill library composition, and the HITL gate model.
- **Practice Command** (Healthcare Clinics) — **LIVE** pilot with an aesthetic clinic in Portugal. Owner-eyes-only on anything touching patient identifiers · re-consent verification before recall outreach · RGPD + FADP + ERS-aware. Practice-management MCP read API (Dentally / Clinicminds / Doctolib / Doctoralia / Optident depending on market).
- **Legal Efficiency Suite** (Law Firms) — **LIVE** with two boutique firms (one Portuguese, one cross-border EU). Supervising-lawyer review on every drafted document · privilege-aware redaction by default · OpenClaw (the autonomous tier) is strictly **not available** for law firms in any of the three markets we operate in — that's the professional-rules ceiling, not a config switch. DMS read (NetDocuments / iManage / SharePoint / filesystem).
- **Field Service Command** (Home Services) — designed and packaged, awaiting first signed pilot. Owner-operator + dispatcher cockpit, Telegram bridge for in-van approvals, GBP review-cascade, n8n WhatsApp Business inbound. OpenClaw available as a later upgrade path.
- **Command Center** (horizontal fallback) — generic SMB owners who don't fit cleanly into the three verticals. 11 core skills only, no vertical extensions, drafts-only everywhere.

**The architectural insight — configure a reliable platform, don't build from scratch.** Same lesson from years of Salesforce delivery consulting: when the platform vendor has invested in making the foundation reliable, the highest-leverage consulting work is configuring that platform for the customer's vertical, not building bespoke alternatives. **Claude Cowork is the platform.** Anthropic owns the cockpit, the MCP integration plane, the runtime, the security model. My job is to configure it for the operator's vertical — wire the right MCP fleet, compose the right skill library, seed the right memory, tune the right HITL gates. **Verticalisation is a configuration concern, not a code concern.** Every engagement is the same shape: Cowork desktop install + configured `CLAUDE.md` + seeded memory + MCP fleet matched to the operator's existing tools + the 11-skill core library + vertical-specific extensions + scheduled tasks for recurring digests. Adding a vertical = adding ~6-7 skills, ~2-3 MCP-server bindings, and a vertical-tuned HITL gate model. **Building agents from scratch was the version-1 mistake**; configuring a reliable platform is the version that ships and sustains.
**Stack — adopted platforms:** Claude Cowork (desktop AI cockpit) · MCP servers (Gmail · Google Calendar · Google Drive · Airtable · Google Business Profile · per-vertical PMS/DMS/dispatch reads) · n8n (WhatsApp Business inbound bridge) · Telegram Bot API (in-van approvals) · Claude API (model tier)
**Stack — my architecture on top:** the 11-skill core library (admin / marketing / sales / accounting-prep) · per-vertical extensions (Practice Command +7 · Legal Efficiency Suite +6 · Field Service Command +7) · 3 classes of HITL gate (owner-eyes-only · supervising-lawyer review · dispatcher-confirms) · the 4-phase ~30-day implementation arc (Discovery → Memory seed → Fleet wiring → Pilot operation) · the canonical discovery workbook · skill-naming-as-API-design discipline
**Case study:** [./claude-cowork/](./claude-cowork/) — 3 vertical templates · skill-library composition · per-vertical HITL gate models · enterprise per-vertical topology · 4-phase implementation arc · vertical regulatory references (RGPD · FADP · ERS · Order-of-Lawyers professional rules · home-services trade regulations)
