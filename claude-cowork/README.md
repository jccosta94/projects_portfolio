...
The consulting practice ships three vertical templates on top of [Claude Cowork](https://claude.com/cowork) (Anthropic's desktop AI app with MCP-tool access): **Practice Command** for healthcare clinics, **Legal Efficiency Suite** for law firms, and **Field Service Command** for home services. Same Cowork install across all three. What varies per vertical is the MCP fleet, the skill library composition, and the human-in-the-loop gate model. No custom code per vertical; everything that's "vertical" is configuration.

Two of the three are live in pilot today: Practice Command at an aesthetic clinic in Portugal, Legal Efficiency Suite with two boutique firms (one Portuguese, one cross-border European). The third is fully designed and packaged but waiting for the first signed pilot. All three sit on top of an 11-skill core library plus per-vertical extensions. The constraint that triggered the architecture work: a one-person integration practice can't credibly sell three different "bespoke AI builds" per vertical and survive the delivery cost. The only way the economics work is if every vertical engagement is the same Cowork environment with a configured fleet on top.
...
**Configure a reliable platform, don't build a custom agent from scratch.** The lesson I bring across from years of Salesforce delivery consulting: when a platform vendor has already invested in making the foundation reliable — the cockpit, the integration plane, the runtime, the security model, the upgrade path — the highest-leverage consulting work is *configuring that platform for the customer's vertical*, not *building bespoke alternatives*. Salesforce consultants don't write their own CRM; they configure Salesforce. The same logic applies here. **Claude Cowork is the platform.** Anthropic has invested in the desktop cockpit, the MCP integration plane, the skill-loading runtime, the scheduled-tasks runner, the security model around tool execution. **My job as a consultant is to configure Cowork for the operator's vertical** — wire the right MCP fleet, compose the right skill library, seed the right memory, tune the right HITL gates — not to ship a from-scratch AI agent per customer. Building agents from scratch was the version-1 mistake; the cost-of-delivery per engagement swallowed the engagement fee, and version 2's build was a snowflake the version-3 build couldn't reuse. **Configuring a reliable platform is the version that ships and sustains** — same operating model that worked for Salesforce CRM delivery, applied to AI cockpit delivery.
